...
- **轮次放大（固定窗口 → N× 轮，P5-bis 实测 2026-08-28，本地 Qwen3.6-35B-A3B，B=16K tok）**：零压缩对照 **20 轮触窗死亡**，双引擎 **60 轮满预算存活未触顶**（活水位峰值仅为窗口 70%）→ **可持续轮数 ≥3.2×（截尾下界）**，探针 6/7（精确依赖 4/4）。缓存口径：剔除灾难性重算请求后的**干净命中率 84.8%，与零压缩对照（84.7%）一致**——逐请求前缀稳定性零劣化；全量命中率差（76.1% vs 83.1%）全部来自 16 次灾难事件（换代关联 11 + 服务端原生 5，详见 [`docs/p5bis-turn-amplification.md`](docs/p5bis-turn-amplification.md) §7.3）。**判别实验已结案（同日，`-np 2` 双槽复跑）**：灾难事件 **16→4（4× 塌缩）**，残余 4 次逐事件归因全部非 aux 驱逐（任务原生/图剪一次性税/服务端底噪）——灾难主群为单槽部署伪影，引擎固有缓存成本=图剪一次性税+底噪（§7.5）。**禁裸引用"数倍"，须带窗口/任务/模型三要素。**
...
`npm run check` = typecheck + smoke + 单测（195/195 全绿，2026-08-29）。每个数字都带产物路径（见 `CHANGELOG.md` 与各 `spike/out/` 实验记录）。

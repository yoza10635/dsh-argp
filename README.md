...
- **轮次放大（固定窗口 → N× 轮，P5-bis 实测 2026-08-28，本地 Qwen3.6-35B-A3B，B=16K tok）**：零压缩对照 **20 轮触窗死亡**，双引擎 **60 轮满预算存活未触顶**（活水位峰值仅为窗口 70%）→ **可持续轮数 ≥3.2×（截尾下界）**，探针 6/7（精确依赖 4/4）。判据与偏差全披露见 [`docs/p5bis-turn-amplification.md`](docs/p5bis-turn-amplification.md) §7（含非压缩轮命中率 −2.9pp 的 G6 未达注记与截尾说明）。**禁裸引用"数倍"，须带窗口/任务/模型三要素。**

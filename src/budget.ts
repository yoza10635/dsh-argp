...
export interface TokenMeter { measure(session: Session): { totalTokens: number; surfaceTokens: number } }
 ...
 */
export function acquireTokenMeter(ctx: Context): TokenMeter | undefined {
  try {
    return (ctx as any).tokenMeter ?? (ctx as any).get?.('tokenMeter')
  } catch {
    return undefined
  }
}
...
export function visibleChars(session: Session): number {
  let total = 0
 ...
 */
export async function resolveScaledBudgets(
  host: BudgetHost,
 ...
 */
export function measureTokens(host: BudgetHost, session: Session, extraTokens = 0): { contextTokens: number; surfaceTokens: number; source: 'anchored' | 'tokenMeter' | 'config' | 'chars' } {
  ...
  if (host.lastRealAnchorSeq >= 0 && host.lastRealPromptTokens > 0) {
    ...
    for (const seq of session.surface.nodes) {
      if (seq > host.lastRealAnchorSeq) deltaChars += eventText(session, seq).length
    }

...
export interface ParsedCite { text: string; level: CiteLevel }
 ...
 */
export function parseCitesBlock(raw: string): ParsedCite[] | null {
  try {
    ...
    for (const item of parsed.cites) {
      ...
      if (item !== null && typeof item === 'object' && typeof (item as { t?: unknown }).t === 'string') {
        const t = (item as { t: string }).t
        const l = (item as { l?: unknown }).l

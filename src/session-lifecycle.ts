...
import { selectManualRanges, compactRegions, type GraphPruneRecord, type PruneTxHost } from './prune-tx.js'
 ...
 */
export function registerSettings(ctx: Context, host: LifecycleHost, config: ArgpGraphConfig): void {
  ...
  ctx.inject(['settings'], (scopedCtx: Context) => {
    ...
    const apply = (): void => { host.argpSettings = host.settingsSource() }
...
export function mountPeratomStack(ctx: Context, host: LifecycleHost, config: ArgpGraphConfig): void {
  if (config.peratom !== undefined && typeof config.peratom === 'object' && config.peratom !== null) {
    ...
    if (declarer !== null) host.injectEdges = (atoms) => declarer.buildInjectEdges(atoms)
    if (compressor !== null) {

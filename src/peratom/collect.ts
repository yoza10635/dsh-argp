...
/** 某轮已规划过的最大 seq（-1 = 未压过）。 */
export function waterMarkOf(host: CollectHost, session: Session, turn: number): number {
  return host.passWatermark.get(session)?.get(turn) ?? -1
}

/** 成功落地后推进水位（单调不回退）。 */
export function advanceWaterMark(host: CollectHost, session: Session, turn: number, endSeq: number): void {
...
export function collectFromWindow(
  host: CollectHost,
  session: Session,
  turn: number,

...
const PROMPT_RULES = [
  ...
  '',
  '## 工具结果压缩（tools）',
  '- level="extract"：关键内容摘录（1-3 句），必须逐字保留路径、行号、错误码、标识符、URL、哈希等 load-bearing 精确串。',
  '- 保真纪律（硬规则）：文件路径、file:line:col 行号定位、URL、UUID、十六进制哈希、全大写错误码、key=value 键值对，这七种形态的串必须原样出现在 text 里——禁止转述、禁止改写分隔符（空格/等号/冒号/井号一律保持原样）。宁可整段抄原文，不要自己组织语言。',

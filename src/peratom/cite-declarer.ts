  ...
  for (const event of events) {
    ...
    if (event.type === 'user/message') {
      ...
      if (turn === closed) {
        ...
        if (isOwnSourceKind(source)) continue
...
export class CiteDeclarer {
   ...
   */
  private dump(row: Record<string, unknown>): void {
   ...
   */
  private dumpCites(record: CiteRecord): void {
  ...
  buildInjectEdges(atoms: Atom[], redirect?: (seq: number) => number | undefined): SemanticEdge[] {
    try {
      ...
      const resolve = (seq: number): [number | undefined, boolean] => {
        const direct = idBySeq.get(seq)
        if (direct !== undefined) return [direct, false]
        if (redirect === undefined) return [undefined, false]
      ...
      for (const cite of this.edgeCache.values()) {
        ...
        if (from === undefined || to === undefined) { droppedByMissingEndpoint += 1; continue }
        if (from === to) { droppedBySelfLoop += 1; continue }
        if (fromRedirected || toRedirected) redirectedEdges += 1

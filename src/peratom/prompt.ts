 ...
 */
export type LlmBackend = { kind: 'dsh-llm'; spec: DshLlmSpec } | { kind: 'fetch'; endpoint: ResolvedEndpoint } | null
...
export function buildPrompt(collect: CurrentTurnCollect): string {
  const atoms: string[] = []
 ...
 */
export function backend(host: PromptHost): LlmBackend {
  if (host.dshLlm !== null) return { kind: 'dsh-llm', spec: host.dshLlm }
  if (host.endpoint !== null) return { kind: 'fetch', endpoint: host.endpoint }
  if (host.autoLlm !== null) return { kind: 'dsh-llm', spec: host.autoLlm }
  return null
}

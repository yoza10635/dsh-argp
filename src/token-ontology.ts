 ...
 */
export const LOAD_BEARING_PATTERNS: RegExp[] = [
  /https?:\/\/\S+/g,                                   // URL
 ...
 */
export function repairWithTrailer(candidate: string, missing: readonly string[]): string {
...
export function trailerText(missing: readonly string[]): string {
 ...
 */
export function hlsRepairEconomics(
  originalLength: number,
  candidateLength: number,
  missing: readonly string[],
  threshold: number = DEFAULT_HLS_ROI_THRESHOLD,
): HlsRepairEconomics {
 ...
 */
export function deriveInferredEdges(
  atoms: readonly OntologyAtom[],
  opts: InferredEdgeOptions = {},
): InferredEdgePair[] {
  ...
  for (let i = 0; i < atoms.length; i += 1) {
    ...
    for (const t of tokensOf.get(a.seq) ?? []) {
      ...
      for (const j of inverted.get(t) ?? []) {
        if (atoms[j].seq >= a.seq) continue // 只连更旧的数据原子

...
1. **单位 token 重要性（预算密度排序）**：设计 §5.4 排序键为 `[edgeLevel, eff, lastRef, seq]`（绝对 eff 升序），忽略原子体积。但压缩本质是"retain 预算内保留最大信息"——该问题的标准贪心解是按 **eff/tokens** 降序剪（分数背包）。落地方案（采用"分级"：eff 同档内插 token 降序，大先剪）：
   ...
   - 诚实限定：eff 只有 5 档粗粒度，增益被粗糙度封顶——预期是"剪枝效率"，非信息保留质变
   - **spike 18 离线实证**：同档大小悬殊场景 B 配置 2 原子达标 vs A 现状 9 原子（效率 4.5×），保留 eff 48 vs 13（3.7×）；均匀场景无差异（合理）
   - **spike 19 真实验证**（纯 A 原子同档，4 小 fact + 4 大 echo）：legacy 剪 7 原子含 **4 fact 全剪**（Q1/Q2 靠 recall 2 次找回）；density 剪 3 原子 **0 fact 剪**（surface 直读，0 recall）——同达成度下保留集价值密度更高、省 recall 调用
   - **⚠️ 排序方向坑**：tokens 需降序（负数入键），升序会先剪小原子反噬
   - **⚠️ 适用边界（spike 19 首跑发现）**：带工具结果的场景测不出密度——A+R 绑定组里 R 的 eff=0 天然拉低组 key，两种模式都会先剪含 R 的组；密度差异只在同档纯 A 原子间显现（A+R 组内差异被 eff=0 主导）

2. **版本链存活代表的重要性叠加**：当前 `findVersionDuplicates`（L534-588）按文本全等去重，older 被预剪、newer 存活，但 **链长（count）未保留**，且存活代表若本身入度 0 会在后续 pass 被当普通原子剪掉 → 整条链信息从 surface 消失（只能 recall 找回）。提案：给版本链记账（`Map<string, {atom, count}>`，已实现于 findVersionDuplicates 返回值），存活代表的 eff 叠加 `(count-1)×w`（w=1）。约束：
   - 叠加只提升 eff 层（edgeLevel 内 tie-break），不跨等级——与密度排序正交可组合
   - 与密度排序有张力：链代表若是大 R，叠加后仍按密度排——需防"被密度淹没"也防"大 token 永不剪"
...
## 6. 方法论注记（2026-08-17，对外叙事准则）

1. **"关键信息"客观性分层**（用户元问题）：三层光谱——语义层（cites 声明/eff 赋值，主观，LLM+设计者判断）、结构层（入度拓扑/链式解锁，半客观，图论可复现）、任务层（U 锚点/评测达成度，客观，可检验）。**"关键"没有脱离任务的绝对定义**（香农熵量信息量不量重要性）；ARGP 的定位不是"更客观地定义关键"，而是"降低主观定义的赌注"。
2. **归因修正（用户批评）**：**"剪错可恢复"归功于 recall 机制（原文保留 + 检索通道），不是剪枝算法**——给摘要引擎挂同样的原文保留+检索，它同样"可恢复"。剪枝的真实贡献有两层，必须分开说：

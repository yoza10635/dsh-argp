...
| 指标 | ON | OFF | 比（OFF/ON） |
|---|---|---|---|
| turns（**会话 turn/start 数，≠ 任务书轮序**） | 24（含 2 个重复注入轮） | 22 | — |
| **原始写入 chars（≈tok）** | **341,329（≈97.5K）** | **1,245,131（≈355.8K）** | **3.65×** |
| atoms（数据原子） | 759（650） | 1097（717） | 1.45× |
| compaction/start | 38 | 39 † | 1.03× |
| compaction/summary | 38 | 39 † | 1.03× |
...
但 **ON 的 tool/result 事件 1.68× OFF**、prune/replace 反而更多。
一个真实有因果的机制不会同时在多个量纲上给出相反符号。

> † **订正（细核产物后，见 §11.12）**：此处初稿写"压缩次数两臂同为 38/38"是**错的** ——
> 误把 harness 收尾 dump 里 OFF 的 38 当成了最终值。从最终 jsonl 精确计数是 **38 vs 39**，
> 且多出来的那 1 次来自**我自己**为补交遗留文件而跑的 `--commit-pending` 续跑。不等就是不等，
> 不能拿"巧合相等"当论据。真实论据只有一条，但已经足够：**分歧符号互相矛盾 + 方向跨批次不稳定**。
...
### 11.12 细核产物后的订正：**§11.11 的逐轮对照表作废**，并新增一条不可比来源（2026-09-17）

触发：用户追问"这是**细核过产物**的结论吗"。复核方式 = **不依赖 spike43 的打印**，
自写脚本从原始 jsonl 重算（`.tmp/audit-xcheck.mjs` / `.tmp/turn-map.mjs`），并按 `agent/inbox/spliced`
（任务书 prompt 的注入事件）做**prompt 锚定**。结论：**部分是细核的，部分是我照抄工具输出 + 推断；
细核后发现 1 个事实错误 + 1 个方法错误。**

#### 复核结论：数字本身复现无误

| 项 | spike43 报的 | 我独立重算 | 判定 |
|---|---|---|---|
| ON 原始 chars | 341,329 | **341,329** | 复现 ✓ |
| OFF 原始 chars | 1,245,131 | **1,245,131** | 复现 ✓ |
| `chars` 口径 | — | 仅 `surfaceOp==='append'`（`isOriginalAppend`），**replace 副本 3834/2549 条已排除** | 确认 ✓ |

⇒ **3.65× 这个核心数字站得住**，不是口径错。

#### 事实错误（已订正）：`compaction/start` **不是 38/38，是 38 vs 39**

初稿拿 harness 收尾 dump 里 OFF 的 38 当最终值。从最终 jsonl 精确计数：**ON=38、OFF=39**。
多出的那 1 次来自**我自己**为补交 OFF 遗留文件而跑的 `--commit-pending` 续跑（该续跑会先跑一次预压检查）。
→ §11.11 里"压缩次数两臂巧合相等"这条论据**删除**。剩下的真实论据只有"分歧符号互相矛盾 + 方向跨批次不稳定"。

#### 方法错误（已作废）：**ON 臂的会话 turn 序与任务书轮序对不上**

| | ON | OFF |
|---|---|---|
| `turn/start` 数 | **24** | 22 |
| 任务书 prompt 注入数 | **24** | 22 |
| `session/end-seed`（**段数**） | **3** | 1 |

用 prompt 文本开头逐字核对，**ON 臂里任务书 T13（BOSS，"1. 每 120 秒刷一个 BOSS…"）的 prompt 被注入了 3 次**
（第 13/14/15 注），OFF 只有 1 次。其中 ON 的**第 14 注落在"退化轮"**上：该轮 **0 次 tool/call、仅 1 个原子（102 字符）、
却发生 254 个 compaction 事件**。⇒ **ON 的 turn N ≠ 任务书 T_N**（自 turn 16 起整体错位 +2），
**§11.11 那张"分轮原始 chars"表是把不同任务轮对比，作废**。

#### 新增不可比来源：**两臂的"会话段数"不对称，而这由我的恢复程序引入**

`session/end-seed` ON=3（`on3-r1` 起跑 + `on3-r3` 因 G3 误杀续跑 + `on3-r4` 因 T22 前补跑），OFF=1（一气跑完）。
这不是臂效应，是**我的恢复过程**造成的：
- **`on3-r3` 这次 resume 把它的首轮 prompt（T13）注入了 3 次、并多产生 2 个 turn 边界**
  （ON 共 24 turn / 24 次注入；`on3-r4` 则未重复注入 —— 所以不是"每次 resume 必然重复"，
  **是 r3 那一次的现场形态**，具体机制未细究，但**数量关系是核过的**：12(r1) + 10(r3: T13×3 + T14..T20) + 2(r4) = 24）；
- 且按 §11.8 的机制纪律，**resume 边界会把 `lastRealPromptTokens` 重置为 0 → 走字符估算路径** ——
  即 **ON 的压缩触发行为有一部分是"resume 造成的"，不是"组件 A 造成的"**。

#### 对 §11.11 结论的影响

- "**C1 不可归因**"**不变，而且更强**：现在有**两条独立理由** ——
  ① 3.65× 内容分叉且分歧符号矛盾；② 段数/轮数不对称 + 重复注入（额外结构混淆）。
- "组件 B 0 落盘"**不受影响**（两臂 `[restored]` 计数均为 0，且归因是配置侧已排除）。
- A1 volume 判定**不受影响**（ON 341K < 400K），但要注意 ON 的 341K 里含 2 次重复 T13 注入带来的量。

#### 新增纪律（下次跑批强制）

1. **A/B 两臂都必须一气跑完，禁止中途 `--resume`。** 若因故障必须 resume，**resume 后必须核对
   `turn/start` 数与 prompt 注入数是否仍等于任务书轮数**；不等则该臂语料**只能做整段分析，不能做逐轮对照**。
2. **逐轮对照一律用 prompt 锚定（`agent/inbox/spliced` 的文本），不要用 `turn/start` 的序号** ——
   turn 序号会因 resume 漂移。

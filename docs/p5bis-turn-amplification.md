...
1. **G6 未达（−2.9pp）**。归因分析（未完，登记待查）：E 臂后段携带 40-56K 单调可缓存前缀（结构性高命中），A 臂刻意压缩使可缓存基数变小（E hit 18344 tok/轮 vs A 13788 tok/轮）；且 A 的 miss 集中在图剪/per-atom replace 的相邻轮（换代后 catalog/原子内容变化）。**前缀稳定性本身另有独立证据**（A 臂逐轮指纹全同、P5 的 21 请求指纹测试），G6 的"E 对照" formulation 把"故意少留上下文"罚成了"缓存不优"——判据设计缺陷与真实成本并存，下一步用 miss/轮而非命中率重述该闸门，并单查换代相邻轮的 miss 归因。**1.0.0 对外引用命中数字时必须带此注记。**

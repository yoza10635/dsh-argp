...
test('tokenMeter: uses ctx.tokenMeter.measure for trigger decision', async () => {
  const { ctx, engine } = await makeEngine()
  try {
    ;(ctx as { tokenMeter?: unknown }).tokenMeter = {
      measure: () => ({ totalTokens: 10000, surfaceTokens: 10000 }),
    }
    const session = Session.create(SessionId('token-meter-test'))
    appendUser(session, 'user anchor')
    appendAssistant(session, 'A1:' + 'x'.repeat(300), 1)
    appendAssistant(session, 'A2:' + 'y'.repeat(300), 2)
    appendAssistant(session, 'A3:' + 'z'.repeat(300), 3)

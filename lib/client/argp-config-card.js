...
function SelectField(props) {
    ...
    return h('div', { style: fieldStyle }, h('div', { style: fieldHeadStyle }, h('label', { htmlFor: id, style: labelStyle }, label), state.overridden
        ...
        value: state.text,
        onChange: (e) => { onEdit(e.target.value); },
    }, ...options.map(opt => h('option', { value: opt, key: opt }, labels[opt] ?? opt))), h('p', { style: hintStyle }, hint));
}
/**
...
export function ArgpConfigCard(props) {
    ...
    return h('li', { style: cardStyle }, h('button', {
        ...
        ? h('div', { style: bodyStyle }, !state.writable
        ...
        }, h('span', { style: chevronStyle(advOpen) }, '▾'), t('advanced')), advOpen
            ? h('div', { style: { paddingLeft: 16, borderLeft: '2px solid var(--dsw-alias-border-l2, rgba(0, 0, 0, 0.1))' } }, h(TextField, {
                id: 'argp-maxPasses',
                label: t('maxPasses'),
            ...
            : null, h('div', { style: footerStyle }, state.failed

...
import { acquireTokenMeter } from './budget.js';
...
import { selectManualRanges, compactRegions } from './prune-tx.js';
 ...
 */
export function normalizeConfig(ctx, host, config) {
    ...
    if (config.tombstoneMergeMinRun !== undefined)
        host.tombstoneMergeMinRun = config.tombstoneMergeMinRun;
    if (config.inferredMinTokenLen !== undefined)
        host.inferredOpts.minTokenLen = config.inferredMinTokenLen;
    if (config.inferredStopwordRatio !== undefined)
        host.inferredOpts.stopwordRatio = config.inferredStopwordRatio;
    if (config.inferredMaxEdgesPerAtom !== undefined)
        host.inferredOpts.maxEdgesPerAtom = config.inferredMaxEdgesPerAtom;
    if (config.inferredWindowTurns !== undefined)
        host.inferredOpts.windowTurns = config.inferredWindowTurns;
}
/**
 ...
 */
export function registerSettings(ctx, host, config) {
    const settingsEntry = {
    ...
    host.argpSettings = settingsEntry;
    host.settingsSource = () => settingsEntry;
    ...
    ctx.inject(['settings'], (scopedCtx) => {
        ...
        host.settingsSource = () => scope.get();
        const apply = () => { host.argpSettings = host.settingsSource(); };
 ...
 */
export function mountPeratomStack(ctx, host, config) {
    if (config.peratom !== undefined && typeof config.peratom === 'object' && config.peratom !== null) {
        ...
        if (declarer !== null)
            host.injectEdges = (atoms) => declarer.buildInjectEdges(atoms);
        if (compressor !== null) {
            host.onOverflowCompress = async (session) => {
            ...
            // 区别只在触发条件：压力 vs 溢出错误）。
            host.onPrePressureCompress = async (session) => {
                await compressor.compressOpenTurn(session);
            };
        }
 ...
 */
export function bindSession(host, session) {
    ...
    if (host.frozenCatalog === null)
        host.frozenCatalog = host.catalogText(20, 70);
}
/**
 ...
 */
export function rebuildLedgerFromLog(host) {
 ...
 * 额度用尽则不再重挂（避免"每步白压"）。
 */
export function rearmReactive(host, session, used) {
    if (used < host.reactiveRetries)
        host.reactivePending.set(session, true);
}
/**
 ...
 */
export async function compactNow(host, agent, signal, sourceCommandId) {
    ...
    try {
        const run = async (agentSignal) => {
            ...
            const ranges = selectManualRanges(host, agent.session);
            return compactRegions(host, ranges, agent, opSignal);
        };

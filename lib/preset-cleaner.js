 ...
 */
export async function cleanShippedPresets(presets, options = {}) {
    const strip = options.strip ?? DEFAULT_STRIP_ROWS;
    const suffix = options.suffix ?? '-argp';
    const outcomes = [];
    const roster = await presets.list();
    const candidates = roster.filter(row => row.trust === 'system'
        ...
        try {
            ...
            if (existing === undefined) {
                ...
                if (refreshed === undefined)
                    throw new Error(`copy reported success but roster lost ${targetId}`);
            ...
            if (deisolated.removed)
                removedList.push('isolate:compaction');

 ...
 * 全部可单测；策略口径与 per-atom-implementation-plan.md P0 一一对应（已迁出公开仓库）。
 *
 ...
 */
export function locateQuotes(quotes, message) {
    const spans = [];
    const misses = [];
    if (!Array.isArray(quotes))
        return { spans, misses: ['<not-an-array>'] };
 ...
 */
export function complementGaps(spans, len, text) {
    const gaps = [];
    let cursor = 0;
 ...
 */
export function resolveSplit(message, quotes, coverageFlip = COVERAGE_FLIP) {
    if (message.length === 0)
        return { kind: 'unsplit', reason: 'empty-message' };

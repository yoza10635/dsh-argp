 ...
 */
export async function completeViaDshLlm(ctx, spec, prompt, timeoutMs, contextMessages, contextTools) {
 ...
 */
export function autoDshLlmSpec(ctx, route) {
    if (route === null)
        return null;
    ...
    if (provider === undefined || provider === '' || model === undefined || model === '')
        return null;
    if (resolveLlmRuntime(ctx) === undefined)
        return null;
    return { provider, model };
}
/** 宿主是否挂了 llm 服务（自动兜底的前提之一；测试/诊断用）。 */
export function hostHasLlm(ctx) {
...
function flattenWireText(blocks) {
    const parts = [];
    for (const block of blocks) {
        if (block.type === 'text')
            parts.push(block.text);
        else if (block.type === 'reasoning')
            continue; // reasoning 走独立字段
        else if (block.type === 'tool-result')
            continue; // tool-result 展开为独立 wire 消息
        else if (block.type === 'tool-call')
            continue; // tool-call 走独立字段
        else
            parts.push(`[${block.type} omitted]`);
    }
    return parts.join('');
}
export function serializeWireMessages(messages, logger) {
    const wire = [];
    ...
    const reasoningChars = wire
        .map(m => (typeof m.reasoning === 'string' ? m.reasoning.length : 0))
        .reduce((a, b) => a + b, 0);
 ...
 */
export function serializeWireTools(tools) {

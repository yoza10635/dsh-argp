...
export function collectCurrentTurn(host, session, afterSeq) {
    ...
    for (const event of events) {
        ...
        if (event.seq > endSeq)
            endSeq = event.seq;
        if (isPrev && event.seq > prevMaxSeq)
            prevMaxSeq = event.seq;
    }
    const collect = collectFromWindow(host, session, closed, turnEvents, startSeq, endSeq);
    if (collect !== null && mergePrev && prevMaxSeq >= 0)
        collect.mergedPrevTurnMaxSeq = prevMaxSeq;
    return collect;
}
/**

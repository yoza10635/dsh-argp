...
import { buildPrompt, postChat } from './prompt.js';
// -- LLM 调用与暂存 ------------------------------------------------------
/**
 ...
 */
export function buildContextPrefix(session) {
 ...
 */
export function resolveEffectiveCtk(host, session) {
    ...
    const ctk = { ...(host.chatTemplateKwargs ?? {}) };
 ...
 */
export function prefixWithinBudget(host, session, promptChars) {
    ...
    for (let i = events.length - 1; i >= 0; i -= 1) {
        ...
        if (estimate > host.prefixBudgetTokens)
            return false;
        return true;
    }
    return false;
}
export async function callAndStash(host, session, collect) {
...
export async function prepareCurrentTurn(host, session) {
    ...
    if (entry.error === undefined && !entry.parseFailed) {
        ...
        if (collect.mergedPrevTurnMaxSeq !== undefined)
            host.advanceWaterMark(session, collect.turn - 1, collect.mergedPrevTurnMaxSeq); // 并入的中断轮水位双推进
    }
    return entry;
}
/** 发射段：把该 session 的全部就绪事务落入下一次 open-turn 窗口（同步追加，吞错记账）。 */
export function flushStashed(host, session) {
    while (true) {
...
export async function compressCurrentTurn(host, session) {
    const collect = host.collectCurrentTurn(session);
 ...
 */
export async function compressOpenTurn(host, session) {
    const collect = host.collectOpenTurn(session);
    return compressCollect(host, session, collect);
}
/** 共享压缩尾部：中断/无候选短路（不推进水位）+ callAndStash + 立即 flush（成功才推进水位）。 */
export async function compressCollect(host, session, collect) {
    if (collect === null)
        return null;
    const chain = buildVersionChainIndex(sessionEvents(session));
    if (collect.interrupted) {
...
export function flushEntry(host, session, collect, decision, record) {
    ...
    // 事务括号配对守卫（2026-09-23）：成功 end 之后仍有可抛语句（断言 2b 代数增量），
    // 一旦抛出，catch 会再补一条 end ⇒ 同一 compactionId 出现**两条** end。写入侧不校验，
    // 加载期 `applyCompactionTransition` 对 end 无分支（首条 end 已清空 trace.compaction），
    // 第二条 end 即命中 invariant 的 "compaction/end has no matching compaction/start"
    // ⇒ 会话**永久打不开**（= 本版本要消灭的"写入无感、重启才炸"同族）。ended 保证全路径恰好一条 end。
    let ended = false;
    ...
    try {
        ...
        if (startIdx < 0 || endIdx < startIdx) {
            throw new Error(`peratom flush: turn window [${collect.startSeq}, ${collect.endSeq}] is not a valid current surface span (startIdx=${startIdx}, endIdx=${endIdx}); refusing to emit a corrupt compaction/summary`);
        }
        if (startIdx === 0) {
        ...
        const textLenBySeq = new Map();
        for (const atom of [...collect.userLong, ...collect.toolResults])
            textLenBySeq.set(atom.seq, atom.text.length);
        ...
        for (const step of plan.steps) {
            ...
            if (step.kind === 'replace') {
                // Shadow-price 契约（宿主 token-meter foldSurfaceProjection）：每条 replace 前各发一条
                // compaction/prune，shadowedRange = 该单原子（与紧随的 replace 范围**严格相等**），否则
                // 重放投影 throw "no adjacent shadow price"——beta.2 的整窗 summary（range=整轮窗口）撞
                // 单原子 replace（range=该原子）即违约（Issue #2 修复引入的回归，0.1.7 新增协议）。
                // 模式同 prune-tx：开头整窗 summary 的 claim 被首条 prune 覆盖、末尾被 off-surface
                // compaction/end 清掉，无契约冲突；每条 prune 的 shadowedSeqs=[该原子] 是单节点连续切片，
                // 同时满足加载期 validateShadowedSeqs。
 ...
 */
export function awaitInFlightPass(host, session) {
    const pass = host.inFlightPass.get(session);
    if (pass === undefined || host.flushWaitMs <= 0)
 ...
 */
export function rememberRoute(host, agent) {
    if (!host.llmAutoEligible)
        return;
    try {

...
export function isAtomCandidate(a, allowInDegree, state) {
    ...
    if (isTombstoneText(a.text) && a.text.length <= TOMBSTONE_MAX_CHARS)
        return false;
    const pos = state.position.get(a.seq);
    if (pos === undefined || pos >= state.recencyCut)
        return false;
 ...
 */
export function compareSortKeys(x, y) {
    const parse = (s) => Number(s.replace(/^0+(?=-)/, ''));
    const xc = x.split('|');
    const yc = y.split('|');
    for (let i = 0; i < xc.length; i += 1) {
        const d = parse(xc[i]) - parse(yc[i]);
        if (d !== 0)
            return d;
    }
    return 0;
...
export function buildTombstones(kept, closureSeqMeta, issuerByCall, pruned, forced) {
    ...
    return kept.map(iv => {
        ...
        if (first !== undefined && metas.every(m => m.closureId === first.closureId)) {
            return { type: 'user', text: closureTombstone(first.closureId, start, end, first.rootPreview) };
        }
        const r0 = iv.atoms[0];
        ...
        return { type: 'user', text: seqRangeTombstone(start, end) };
    });
}
 ...
 */
export function selectClosureToMerge(host, session, atoms, edges, inDegree, askCover, latestTurn, alreadyPruned) {
    const roots = atoms

...
const ARG_SETTINGS_NS = ARG_SETTINGS_KEY;
...
export const EDGE_WEIGHTS = { critical: 10, supporting: 5, contextual: 2, inferred: 1 };
const LEVEL_ORDER = { isolated: 0, contextual: 1, supporting: 2, critical: 3 };
...
export class ArgpGraphEngine extends CompactionEngine {
    ...
    constructor(ctx, config = {}) {
        ...
        try {
            this.tokenMeter = ctx.tokenMeter ?? ctx.get?.('tokenMeter');
        }
        catch {
        ...
        ctx.inject(['settings'], (scopedCtx) => {
            ...
            const apply = () => { this.argpSettings = this.settingsSource(); };
        ...
        // 三级触发（1.4.0）：① 轮初主动（默认承担主力）② 轮中只反应式（输出被钳/overflow）。
        // 轮内主动压缩默认关闭——实测近乎无效（turnGuard 保护当前轮）却每次都在轮中断一次
        // 前缀缓存；`midTurnActive: true` 恢复 1.3.x 行为，供对照实验。
        this.midTurnActive = config.midTurnActive ?? false;
        ...
        if (config.tombstoneMergeMinRun !== undefined)
            this.tombstoneMergeMinRun = config.tombstoneMergeMinRun;
        if (config.inferredMinTokenLen !== undefined)
            this.inferredOpts.minTokenLen = config.inferredMinTokenLen;
        if (config.inferredStopwordRatio !== undefined)
            this.inferredOpts.stopwordRatio = config.inferredStopwordRatio;
        if (config.inferredMaxEdgesPerAtom !== undefined)
            this.inferredOpts.maxEdgesPerAtom = config.inferredMaxEdgesPerAtom;
        if (config.inferredWindowTurns !== undefined)
            this.inferredOpts.windowTurns = config.inferredWindowTurns;
        ...
        if (config.peratom !== undefined && typeof config.peratom === 'object' && config.peratom !== null) {
            ...
            if (declarer !== null)
                this.injectEdges = (atoms) => declarer.buildInjectEdges(atoms);
            if (compressor !== null) {
                this.onOverflowCompress = async (session) => {
                ...
                // 区别只在触发条件：压力 vs 溢出错误）。
                this.onPrePressureCompress = async (session) => {
                    await compressor.compressOpenTurn(session);
                };
            }
        ...
        ctx.on('session/event', (session, event) => {
            ...
            if (event.type === 'assistant/message') {
                ...
                if (finish?.type === 'finish' && finish.reason?.kind === 'max-tokens'
                    && typeof output === 'number' && typeof budget === 'number' && output < budget) {
                    this.reactivePending.set(session, { attempts: 0 });
                    this.log.warn('[argp-graph] output clamped by the host: finish=max-tokens with output='
                        + output + ' < requested ' + budget + ' ⇒ scheduling a reactive prune at the next pre-step');
                }
            }
            ...
            if (event.type === 'compaction/start') {
                ...
                if (typeof cid === 'string' && !cid.startsWith('argp-')) {
                    this.log.warn(`[argp-graph] foreign compaction detected (id=${cid}, turn=${event.data.turn ?? '?'})`
        ...
        ctx.on('agent/request-error', async ({ agent, failure, signal }, next) => {
            ...
            if (signal.aborted || session.surface.replaceGeneration <= genBefore)
                return next();
            if (result !== null) {
                ctx.logger.info(`argp-graph context-overflow step-${isStepOne ? 1 : 3} prune: shadowed ${result.shadowedSeqs.length} surface nodes `
        ...
        ctx.on('agent/pre-step', async ({ agent, messages, step, signal }, next) => {
            ...
            if (!signal.aborted) {
                ...
                //   · step === 1 → L1 轮初主动压缩（主力，阈值沿用 windowRatio 0.8）。
                //   · step  > 1 → 轮内。默认**不做**主动压缩（见 midTurnActive 注释：实测近乎
                //     无效——turnGuard 保护当前轮，209K 只剪得动 1 原子/154 tok——而它每次落地
                //     都是一次轮中 surface 替换 = 断前缀缓存）。轮内只留 L2 反应式。
                // 顺序：先 per-atom 压 open turn（原子降熵），再图剪；两者落在同一个 pre-step
                // 窗口 = 单变异窗口。回调失败隔离：吞错后照常走图剪。
                const turnStart = step === 1;
                if (turnStart || this.midTurnActive) {
                    const incoming = this.incomingTokens(messages);
                    // per-atom 压缩需要 open turn 的原子；闭合轮（无 open turn）归 idle 边界路径，
                    // 此处不重复处理。图剪不受该守卫约束（无非 open turn 也可剪，与 1.3.x 一致）。
                    if (this.onPrePressureCompress !== undefined && this.detectOpenTurn(session) !== null) {
                ...
                // L2 反应式补救：轮中唯一允许的压缩时机（输出被外部钳制之后）。
    ...
    catalogText(maxItems = 20, snippetChars = 70, tokenBudget = 600) {
        ...
        for (const seq of shadowed) {
            ...
            if (usedChars + clipped.length > charBudget && entries.length > 0)
                break;
    ...
    recall(seq) {
        if (this.session === null)
            return null;
        if (!this.shadowedSeqsOf(this.session).has(seq))
            return null;
        const text = eventText(this.session, seq);
    ...
    atomize(session) {
        ...
        for (const seq of session.surface.nodes) {
            ...
            if (event.type === 'assistant/message') {
                ...
                if (cites.length > 0)
                    this.citeStats.declared += cites.length;
    ...
    queryNGramCandidates(index, prefix) {
        const n = this.ngramN;
        if (prefix.length < n)
            return null;
    ...
    selectClosureToMerge(session, atoms, edges, inDegree, askCover, latestTurn, alreadyPruned) {
        ...
        const lastRefByClosure = new Map();
        const inDegreeByClosure = new Map();
        ...
        for (const [id, root] of rootByClosure) {
            if (root.seq === lastRootSeq)
                continue;
            const lastRecalled = this.closureLastRecalled.get(root.seq);
            if (lastRecalled !== undefined && latestTurn - lastRecalled < k)
                continue;
            const lastRef = lastRefByClosure.get(id) ?? 0;
            if (lastRef > latestTurn - k)
                continue;
            if ((inDegreeByClosure.get(id) ?? 0) > 0)
                continue;
        ...
        for (const seq of surfaceSeqs) {
            if (!chosenSet.has(seq)) {
                if (current.length > 0) {
                    const intervalAtoms = current.map(s => bySeq.get(s)).filter((a) => a !== undefined);
    ...
    tryPruneClosures(session, atoms, edges, inDegree, askCover, latestTurn) {
        // 2026-08-22：选择逻辑抽到 selectClosureToMerge（供 compactIfNeeded 降级链并入 pruned 复用），
        // 本方法保持"独立闭包事务"语义（手动/独立路径）；执行段不变。
        const chosen = this.selectClosureToMerge(session, atoms, edges, inDegree, askCover, latestTurn, new Set());
        if (chosen === null)
            return null;
        const intervals = chosen.intervals;
        for (const iv of intervals) {
            for (const a of iv.atoms) {
                const citedBySeq = edges.filter(e => e.to === a.id).map(e => atoms[e.from]?.seq).filter((x) => x !== undefined);
    ...
    consolidateTombstones(session) {
        if (this.tombstoneMergeMinRun <= 0)
            return 0;
    ...
    async runReactivePrune(agent, signal, ctx) {
        const pending = this.reactivePending.get(agent.session);
        if (pending === undefined || signal.aborted)
            return;
        if (pending.attempts >= this.reactiveRetries) {
            this.reactivePending.delete(agent.session);
            this.log.warn('[argp-graph] reactive prune retries exhausted after ' + pending.attempts
                + ' attempt(s); leaving further recovery to the overflow path');
            return;
        }
        pending.attempts += 1;
        const relax = pending.attempts > 1;
        const previousOverride = this.guardOverride;
        if (relax) {
            this.guardOverride = { recencyGuard: 0, turnGuard: 0 };
            this.log.warn('[argp-graph] reactive prune attempt ' + pending.attempts
        ...
        try {
            ...
            if (result !== null) {
                this.log.info('[argp-graph] reactive prune attempt ' + pending.attempts + ': shadowed '
                ...
                this.log.warn('[argp-graph] reactive prune attempt ' + pending.attempts
                    + ': nothing prunable (candidate set exhausted)');
            }
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
    ...
    incomingTokens = 0) {
        ...
        const isAtomCandidate = (a, allowInDegree) => {
            if (a.type === 'U' && a.sourceSeq === undefined) {
                ...
                if (pos === undefined || pos >= recencyCut)
                    return false;
                if (a.turn > latestTurn - this.turnGuard)
                    return false;
                // 动态复核：所有保留入边都必须来自覆盖者，否则豁免失效
                const incoming = edges.filter(e => e.to === a.id);
                if (incoming.length === 0 || incoming.some(e => e.from !== coverer))
                    return false;
                return true;
            }
            ...
            if (a.type === 'A' && a.toolCallIds.length > 0) {
                const groupIds = new Set([a.id]);
                const groupRs = atoms.filter(x => x.type === 'R' && a.toolCallIds.includes(x.toolCallIds[0] ?? ''));
                for (const r of groupRs)
                    groupIds.add(r.id);
                if (groupRs.length > 0) {
                    const aCitesR = edges.some(e => e.from === a.id && groupRs.some(r => e.to === r.id));
                    // R 的外部入边：语义边来自组外原子，或确定性边来自组外原子（其他 A 调用了同一 callId 链）
                    const anyRExternalIncoming = groupRs.some(r => (curInDegreeDecl.get(r.id) ?? 0) > 0 || // 语义**声明**入度（cites/inject）——排除 inferred（见上方实验注释）
                        deterministicEdges.some(e => e.to === r.id && !groupIds.has(e.from))); // 确定性：组外 A→R
                    if (!aCitesR && !anyRExternalIncoming)
                        return false;
                }
            }
            if (!allowInDegree && (curInDegree.get(a.id) ?? 0) > 0)
                return false;
            return true;
        };
        const isGroupCandidate = (g, allowInDegree) => g.every(a => isAtomCandidate(a, allowInDegree));
        ...
        for (let pass = 0; pass < this.maxPasses; pass += 1) {
            ...
            const groupKey = (g) => g.map(sortKey).sort()[0];
        ...
        for (const seq of prunedSeqs) {
            ...
            const isSoloR = a.type === 'R' && a.toolCallIds[0] !== undefined
                && (() => {
                    const issuer = issuerByCall.get(a.toolCallIds[0]);
                    return issuer !== undefined && !pruned.has(issuer.id);
                })();
            const lastInterval = intervals[intervals.length - 1];
            const prevPos = lastInterval !== undefined ? position.get(lastInterval.seqs[lastInterval.seqs.length - 1]) : undefined;
        ...
        for (const iv of keptRaw) {
            ...
            const rest = { seqs: [], chars: 0, atoms: [], hasSoloR: false };
            for (const a of iv.atoms) {
                ...
                if (soloHere)
                    rescued.push({ seqs: [a.seq], chars: a.text.length, atoms: [a], hasSoloR: true });
                else {
                    rest.seqs.push(a.seq);
                    rest.chars += a.text.length;
                    rest.atoms.push(a);
                }
            }
            if (rest.chars >= this.minSpanChars)
                kept.push(rest);
        }
        kept.push(...rescued);
        kept.sort((x, y) => x.seqs[0] - y.seqs[0]);
        ...
        const tombstoneTexts = kept.map(iv => {
            ...
            if (first !== undefined && metas.every(m => m.closureId === first.closureId)) {
                return { type: 'user', text: '[elided closure ' + first.closureId
                        + ' seqs=' + iv.seqs[0] + '..' + iv.seqs[iv.seqs.length - 1]
            ...
            return { type: 'user', text: '[elided seq=' + iv.seqs[0] + '..' + iv.seqs[iv.seqs.length - 1]
    ...
    rebuildLedgerFromLog() {
        ...
        for (let i = 0; i < events.length; i += 1) {
            ...
            if (event === undefined)
                continue;
            if (event.type === 'compaction/start') {
                starts.push({ seq: i, compactionId: String(event.data.compactionId ?? ''), lifecycle: event.data });
            }
                ...
                if (s !== undefined)
                    endByStart.set(s.seq, { endSeq: i, error: typeof d.error === 'string' ? d.error : undefined });
            }
        }
        ...
        const typeOfSeq = (seq) => {
            ...
            if (event === undefined)
                return 'X';
            if (event.type === 'user/message')
                return classifyUserMessage(event.data);
            if (event.type === 'assistant/message')
                return 'A';
            if (event.type === 'tool/result')
                return 'R';
        ...
        for (const s of starts) {
            ...
            const intervalRecords = intervalSeqs.length > 0
                ? [{ start: intervalSeqs[0], end: intervalSeqs[intervalSeqs.length - 1], tombstoneSeq: end.endSeq }]
                : [];
            const prunedAtoms = intervalSeqs.map(seq => ({ id: seq, type: typeOfSeq(seq), seq }));
            this.records.push({
                at: String(s.lifecycle.at ?? ''),

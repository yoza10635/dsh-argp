...
export class ArgpGraphEngine extends CompactionEngine {
    ...
    get recencyGuard() { return this.argpSettings.recencyGuard; }
    get turnGuard() { return this.argpSettings.turnGuard; }
    ...
    measureTokens(session) {
        ...
        if (this.lastRealAnchorSeq >= 0 && this.lastRealPromptTokens > 0) {
            ...
            return { contextTokens: this.lastRealPromptTokens + deltaTokens, surfaceTokens, source: 'anchored' };
        }
        ...
        if (this.tokenMeterFn !== undefined)
            return { ...this.tokenMeterFn(session), source: 'config' };
        return { contextTokens: surfaceTokens, surfaceTokens, source: 'chars' };
    }
     ...
     */
    async isPressureExceeded(agent) {
        const session = agent.session;

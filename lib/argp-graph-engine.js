 ...
 */
export function isAtomCandidate(a, allowInDegree, state) {
    if (a.type === 'U' && a.sourceSeq === undefined) {
        ...
        if (pos === undefined || pos >= state.recencyCut)
            return false;
        if (a.turn > state.latestTurn - state.turnGuard)
            return false;
    ...
    if (a.type === 'A' && a.toolCallIds.length > 0) {
        ...
        if (groupRs.length > 0) {
            ...
            const anyRExternalIncoming = groupRs.some(r => (state.curInDegreeDecl.get(r.id) ?? 0) > 0 || // 语义**声明**入度（cites/inject）——排除 inferred（见上方实验注释）
                state.deterministicEdges.some(e => e.to === r.id && !groupIds.has(e.from))); // 确定性：组外 A→R
            if (!aCitesR && !anyRExternalIncoming)
                return false;
        }
    }
    if (!allowInDegree && (state.curInDegree.get(a.id) ?? 0) > 0)
        return false;
    return true;
}
/** 组候选判定（原 isGroupCandidate 闭包）：组内全部原子均候选。 */
export function isGroupCandidate(g, allowInDegree, state) {
 ...
 */
export function sortKey(a, state) {
 ...
 */
export function mergeIntervals(pruned, position, issuerByCall, minSpanChars) {
 ...
 */
export function buildTombstones(kept, closureSeqMeta, issuerByCall, pruned, forced) {
    return kept.map(iv => {

...
export const EDGE_WEIGHTS = { critical: 10, supporting: 5, contextual: 2 };
...
export class ArgpGraphEngine extends CompactionEngine {
        ...
        const isAtomCandidate = (a, allowInDegree) => {
            ...
            if (a.type === 'A' && a.toolCallIds.length > 0) {
                ...
                if (groupRs.length > 0) {
                    ...
                    const anyRExternalIncoming = groupRs.some(r => (curInDegree.get(r.id) ?? 0) > 0 || // 语义入度（cites）——已含组外来源
    ...
    pruneIntervals(session, intervals, semanticEdges, candidateCount, forced, tombstones) {

...
export class ArgpGraphEngine extends CompactionEngine {
        ...
        for (const seq of session.surface.nodes) {
            ...
            if (event.type === 'assistant/message') {
                ...
                if (Array.isArray(stored)) {
                    cites = stored.every(c => typeof c === 'object' && c !== null && typeof c.t === 'string')
                        ? stored.map(c => ({ text: c.text, level: c.level }))
                        : stored.map(t => ({ text: t, level: 'supporting' }));
                }
                else {
                    cites = parsed.cites;
                }
    ...
    pruneIntervals(session, intervals, semanticEdges, candidateCount, forced, tombstoneTexts) {

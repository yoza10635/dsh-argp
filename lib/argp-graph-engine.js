...
export class ArgpGraphEngine extends CompactionEngine {
    ...
    constructor(ctx, config = {}) {
            ...
            execute: async (args) => {
                ...
                for (const seq of seqs) {
                    ...
                    if (event.type === 'user/message') {
                        type = data?.source?.kind === 'plugin' ? 'X' : 'U';
                    }
        ...
        ctx.on('agent/request-error', async ({ agent, failure, signal }, next) => {
            ...
            if (signal.aborted || agent.session.surface.replaceGeneration <= generation)
                return next();
            if (result !== null) {
                ctx.logger.info(`argp-graph context-overflow recovery: shadowed ${result.shadowedSeqs.length} surface nodes `
    ...
    shadowedSeqsOf(session) {
        ...
        for (let index = this.shadowedScanned; index < session.events.length; index += 1) {
            ...
            if (op !== undefined && op !== 'append' && typeof op === 'object'
                && op.op === 'replace'
                && !Array.isArray(event.data.argpCites)) {
    ...
    findVersionDuplicates(atoms, inDegree) {
        ...
        const addPair = (a) => {
            ...
            for (const cid of a.toolCallIds) {
                const r = rByCall.get(cid);
                if (r !== undefined && (inDegree.get(r.id) ?? 0) === 0)
                    dupIds.add(r.id);
            }
        };
        const seenA = new Map();
    ...
    async compactIfNeeded(agent, trigger, _signal) {
        ...
        const selfImportance = (a) => (a.type === 'A' ? 5 : a.type === 'U' ? 3 : 0);

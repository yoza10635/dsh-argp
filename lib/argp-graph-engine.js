...
import { isGroupCandidate, sortKey, compareSortKeys, mergeIntervals, buildTombstones, selectClosureToMerge } from './prune-selection.js';
...
export class ArgpGraphEngine extends CompactionEngine {
     ...
     */
    get windowRatio() { return this.args.windowRatio.get(); }
    get retainRatio() { return this.args.retainRatio.get(); }
    /**
     * 守卫读取点统一走 getter：反应式补救（L2）在第 2 次尝试时用 `guardOverride` 临时
     * 放宽守卫（连当前轮一起剪），使"被钳后回线"成为可能；其余时刻恒等于设置页值。
     */
    guardOverride = null;
    get recencyGuard() { return this.guardOverride?.recencyGuard ?? this.args.recencyGuard.get(); }
    get turnGuard() { return this.guardOverride?.turnGuard ?? this.args.turnGuard.get(); }
    get minSpanChars() { return this.args.minSpanChars.get(); }
    get charsPerToken() { return this.args.charsPerToken.get(); }
    get maxPasses() { return this.args.maxPasses.get(); }
    get enableSummarize() { return this.args.enableSummarize.get(); }
    get sortMode() { return this.args.sortMode.get(); }
    maxOverflowRetries;
    ...
    constructor(ctx, config = {}) {
        ...
        ctx.systemPrompt.section({
            ...
            text: () => 'Context compression (ARGP):\n'
                ...
                // tool/result 里看不到 seq，不带则召回无参数可调（通路实际是断的）。
                + '- A tool result may also have been replaced by a shorter compressed copy. Such a copy is marked at its top with [已压缩-摘取 seq=N] (a verbatim extract — a subset of the original, not the whole output) or [已压缩-摘要 seq=N] (a paraphrase — its wording is NOT the original). It is never the complete original: call recall_detail(N) or recall_pruned(N) for the verbatim text before you quote it or rely on exact strings.',
        });
        ...
        ctx.on('agent/turn-stopping', async ({ agent, signal }) => {
            ...
            try {
                agent.steer(createUserMessage({
                    ...
                    // 宿主/本引擎都把非 `user` 源归为 X（可见、不参剪）⇒ 这条提示既进请求又不会被剪掉，
    ...
    incomingTokens = 0) {
        ...
        // 1.5.1：保留目标 = 可压缩内容 × retainRatio（恒定压缩率），不再用含固定开销的
        // windowTokens 做基数。visible（原子文本和）= 完整请求 − 固定开销（system+tools+MCP），
        // 天然排除不可剪前缀；旧公式 retain = window×0.2 = 0.16W 把固定开销算进基数，
        // 与"目标量 visible 不含固定开销"不对称（触发线含、目标线不含）。
        // 显式 retainTokens 保留旧绝对值语义（逃生阀：配置了绝对目标的用户行为不变）。
        const initialVisible = atoms.reduce((s, a) => s + a.text.length, 0);
        const retainChars = this.explicitRetainTokens
            ? retainTokens * this.charsPerToken
            : Math.floor(initialVisible * this.retainRatio);
        ...
        let forced = false;
        // 无进展固定点检测（安全网）：pruned 单调增、每 pass 结果仅由 pruned 决定 ⇒
        // 某 pass 未新增任何原子即达真固定点，再跑必空转（O(1)/pass 判停）。
        // 正常终止靠「压缩率达标 / 全保护」，此判停只兜底任何残留空转。
        let prevPrunedSize = pruned.size;
        for (let pass = 0; pass < this.maxPasses; pass += 1) {
            if (pass > 0 && pruned.size === prevPrunedSize)
                break;
            prevPrunedSize = pruned.size;
            ...
            if (candidateGroups.length === 0) {
                ...
                if (this.degradationStrategy === 'fail')
                    return null;
                // alreadyPruned 必须是 **seq** 集合（selectClosureToMerge 内部按 seq 过滤 prunableSeqs）——
                // pruned 以原子 id 为键，直接传 pruned.keys()（id 集合）会因 id≠seq 永不命中，
                // 导致「全已剪」闭包被反复选中 → 空闭包 continue 死转、force_prune 饿死（2026-09-22 实锤）。
                const prunedSeqs = new Set();
                for (const a of pruned.values())
                    prunedSeqs.add(a.seq);
                const closure = this.selectClosureToMerge(session, atoms, edges, inDegree, askCoverage, latestTurn, prunedSeqs);
                if (closure !== null) {
            ...
            // 的结构保护换成"整组退场"，配对不变式从此由本处与下方 drag 共同承担。
            {
                ...
                candidateGroups = candidateGroups.filter(g => g.every(a => !(a.type === 'A' && a.toolCallIds.some(cid => rByCallForPrune.get(cid) === undefined))));
        ...
        for (let groupGuard = 0; groupGuard < 8; groupGuard += 1) {
            ...
            for (const iv of kept) {
                for (const a of iv.atoms) {
                    ...
                    for (const cid of a.toolCallIds) {
                        const r = rByCallForPrune.get(cid);
                        if (r !== undefined && !keptSeqs.has(r.seq)) {
                            violators.push(a);
                            break;
                        }
                    }

import { sessionEvents, eventText, recallFromLog, nodeStateOf } from './log-access.js';
 ...
 * 经 host 重赋值落到真实字段）。
 */
export function shadowedSeqsOf(host, session) {
...
export function catalogText(host, maxItems = 20, snippetChars = 70, tokenBudget = 600) {
 ...
 *
 * 原 class 方法；this.x → host.x。 */
export function recallQuery(host, query, maxResults = 5) {
    if (host.session === null)
 ...
 * 原 class 方法；this.x → host.x。
 */
export function recall(host, seq) {
    if (host.session === null)
        return null;
    if (!shadowedSeqsOf(host, host.session).has(seq))
        return null;
    const text = eventText(host.session, seq);
 ...
 * 原 class 方法；this.x → host.x。
 */
export function recallAnyState(host, seq) {
    if (host.session === null)
        return null;
    const shadowed = shadowedSeqsOf(host, host.session);
 ...
 *
 * 原 class 方法；this.x → host.x。 */
export function nodeState(host, seq) {
    if (host.session === null)
        return null;
    const shadowed = shadowedSeqsOf(host, host.session);
 ...
 */
export function latestTurnOf(host, session) {
    let max = 0;
...
export function latestTurnOfSession(host) {
    if (host.session === null)
        return 0;
 ...
 */
export function noteRecallHit(host, seq) {
 ...
 */
export function budgetRecallText(host, text) {
